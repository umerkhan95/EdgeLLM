"""
SIMD-Optimized LLaMA 2 Inference in Pure Mojo
Uses UnsafePointer for SIMD reads, List indexing for writes.
"""
...
struct Matrix:
    """Matrix with SIMD-optimized access."""
...
struct TransformerWeights:
    ...
    fn __init__(out self, path: String, config: Config) raises:
        ...
        f.close()
        print("Loaded", config.vocab_size * config.dim // 1000000, "M embedding params")
...
fn rmsnorm_simd(mut out_mat: Matrix, out_offset: Int,
    ...
    # SIMD normalize (write using List indexing)
    i = 0
    while i + SIMD_WIDTH <= size:
        var x = x_ptr.load[width=SIMD_WIDTH](x_offset + i)
        var w = w_ptr.load[width=SIMD_WIDTH](w_offset + i)
        var result = w * x * ss
        # Write back element by element (can't use store without mut pointer)
...
fn softmax_simd(mut mat: Matrix, offset: Int, size: Int):
    ...
    var i = 0
    while i + SIMD_WIDTH <= size:
        var v = mat.data.unsafe_ptr().load[width=SIMD_WIDTH](offset + i)
        var result = v * inv_sum
        for j in range(SIMD_WIDTH):
            mat[offset + i + j] = result[j]
        i += SIMD_WIDTH
    while i < size:
...
fn matmul_simd(mut out_mat: Matrix, out_offset: Int,
    ...
    """SIMD matrix-vector multiplication - the key hot path."""
    var x_ptr = x_mat.data.unsafe_ptr()
    var w_ptr = w_mat.data.unsafe_ptr()
...
fn transformer_forward(
    ...
    for layer in range(config.n_layers):
        ...
        matmul_simd(state.q, 0, state.xb, 0, weights.wq, wq_offset, dim, dim)
        ...
        # Multihead attention with SIMD dot products
        ...
        for h in range(n_heads):
            ...
            for t in range(pos + 1):
                ...
                # SIMD dot product for attention score
        ...
        matmul_simd(state.xb2, 0, state.xb, 0, weights.wo, wo_offset, dim, dim)
        ...
        matmul_simd(state.hb, 0, state.xb, 0, weights.w1, w1_offset, hidden_dim, dim)
        matmul_simd(state.hb2, 0, state.xb, 0, weights.w3, w3_offset, hidden_dim, dim)
        ...
        matmul_simd(state.xb, 0, state.hb, 0, weights.w2, w2_offset, dim, hidden_dim)
    ...
    # Classifier
    if config.shared_weights:
        matmul_simd(state.logits, 0, state.xb, 0, weights.token_embedding, 0, config.vocab_size, dim)
    else:
        matmul_simd(state.logits, 0, state.xb, 0, weights.wcls, 0, config.vocab_size, dim)
...
fn main() raises:
    ...
    print("Loading SIMD-optimized model from", checkpoint)
